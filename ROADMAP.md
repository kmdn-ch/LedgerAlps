...
| 5 | **Notes de crédit — trace comptable et lien** | ✅ Le montant est juste depuis la v1.4.3 : le signe est appliqué à l'agrégation, une note de crédit réduit la dette d'impôt (LTVA art. 41). Reste ce qui manque **autour** du montant.<br>**Lien vers la facture corrigée** : aujourd'hui une note de crédit ne référence rien. Un contrôleur qui voit une TVA réduite ne peut pas retrouver la facture d'origine, alors que le CO art. 957a al. 2 ch. 5 exige la traçabilité des enregistrements. L'art. 27 al. 4 LTVA décrit d'ailleurs la correction comme « un document qui mentionne et annule la facture d'origine » — mentionner suppose de pointer.<br>**Écriture au journal à la comptabilisation** : comme pour les factures fournisseurs (priorité 1), le document existe sans atteindre les livres. Contrepassation du produit et de la TVA collectée.<br>**Garde-fou sur le montant** : rien n'empêche de créditer plus que la facture d'origine. Une fois le lien posé, c'est vérifiable.
